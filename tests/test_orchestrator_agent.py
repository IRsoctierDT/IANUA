...
@pytest.mark.unit
def test_process_log_returns_all_keys():
...
@pytest.mark.unit
def test_process_log_soc_event_type():
    agent = OrchestratorAgent()
    result = agent.process_log("Failed password for root from 10.0.0.5 port 22 ssh2")
...
@pytest.mark.unit
def test_process_log_threat_intel_populated(tmp_path: Path):
    agent = OrchestratorAgent()
    result = agent.process_log("Failed password for root from 10.0.0.5 port 22 ssh2")
...
@pytest.mark.unit
def test_process_log_no_indicators_when_no_ip():
    agent = OrchestratorAgent()
